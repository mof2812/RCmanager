...
//		else
	...
	case 'V':
		if(strcmp((char*)m_szInstruction,"VERSION") == 0)			/* Version */
		{
			...
			CCONFIG_SERIAL_COM.print(F("Version: "));	
			CCONFIG_SERIAL_COM.print(Version.GetVersion());	
			CCONFIG_SERIAL_COM.println("");
#else
...
uint32_t CInstructionDecoder::EvaluateInstruction_GSWAF(void)
{
/* Variable declarations */
	uint32_t u32RetVal;
	int16_t i16Parameter[CINSTRUCTIONDECODER_MAX_PARAMETERS];
...
/* Implementation */
	...
	if (i16Parameter[0] < CCONFIG_RELAYMODULE_IOS)
	{
		u32RetVal = Timer.GetAsynchronousMode((uint8_t)i16Parameter[0], bData);
...
uint32_t CInstructionDecoder::EvaluateInstruction_GSWC(void)
{
/* Variable declarations */
	uint32_t u32RetVal;
	int16_t i16Parameter[CINSTRUCTIONDECODER_MAX_PARAMETERS];
...
/* Implementation */
	...
	if (i16Parameter[0] < CCONFIG_RELAYMODULE_IOS)
	{
		u32RetVal = Timer.GetCounter((uint8_t)i16Parameter[0], u32Counts);
	
		if (u32RetVal == 0L)
		{
#ifdef CCONFIG_DOCKLIGHT_MODE
			CCONFIG_SERIAL_COM.print("Impulsbetrieb von Schaltkanal ");
			CCONFIG_SERIAL_COM.print(i16Parameter[0]);
...
uint32_t CInstructionDecoder::EvaluateInstruction_GSWCA(void)
{
/* Variable declarations */
	uint32_t u32RetVal;
	int16_t i16Parameter[CINSTRUCTIONDECODER_MAX_PARAMETERS];
...
/* Implementation */
	...
	if (i16Parameter[0] < CCONFIG_RELAYMODULE_IOS)
	{
		...
		if (u32RetVal == 0L)
		{
			...
			CCONFIG_SERIAL_COM.print(u32Counts);
			CCONFIG_SERIAL_COM.print(" Impulse im Impulsbetrieb von Schaltkanal ");
			CCONFIG_SERIAL_COM.println(i16Parameter[0]);
#else
			PrintData(u32Counts);
#endif /* CCONFIG_DOCKLIGHT_MODE */
		}
...
uint32_t CInstructionDecoder::EvaluateInstruction_GSWE(void)
{
/* Variable declarations */
	uint32_t u32RetVal;
	int16_t i16Parameter[CINSTRUCTIONDECODER_MAX_PARAMETERS];
...
/* Implementation */
	...
	if (i16Parameter[0] < CCONFIG_RELAYMODULE_IOS)
	{
		u32RetVal = Timer.GetEnable((uint8_t)i16Parameter[0], bEnabled);
...
uint32_t CInstructionDecoder::EvaluateInstruction_GSWIF(void)
{
/* Variable declarations */
	uint32_t u32RetVal;
	int16_t i16Parameter[CINSTRUCTIONDECODER_MAX_PARAMETERS];
...
/* Implementation */
	...
	if (i16Parameter[0] < CCONFIG_RELAYMODULE_IOS)
	{
		u32RetVal = Timer.GetImmediateMode((uint8_t)i16Parameter[0], bData);
...
uint32_t CInstructionDecoder::EvaluateInstruction_GSWM(void)
{
/* Variable declarations */
	uint32_t u32RetVal;
	int16_t i16Parameter[CINSTRUCTIONDECODER_MAX_PARAMETERS];
...
/* Implementation */
	...
	if (i16Parameter[0] < CCONFIG_RELAYMODULE_IOS)
	{
		if(Timer.GetMode(i16Parameter[0], Mode) == 0)
		{
#ifdef CCONFIG_DOCKLIGHT_MODE
			switch (Mode)
			{
			case CTIMERPIN_MODE_OFF:
				CCONFIG_SERIAL_COM.print("Betriebsmodus von Schaltkanal ");
				CCONFIG_SERIAL_COM.print(i16Parameter[0]);
...
uint32_t CInstructionDecoder::EvaluateInstruction_GSWNF(void)
{
/* Variable declarations */
	uint32_t u32RetVal;
	int16_t i16Parameter[CINSTRUCTIONDECODER_MAX_PARAMETERS];
...
/* Implementation */
	...
	if (i16Parameter[0] < CCONFIG_RELAYMODULE_IOS)
	{
		u32RetVal = Timer.GetInvertedMode((uint8_t)i16Parameter[0], bData);
...
uint32_t CInstructionDecoder::EvaluateInstruction_GSWT(void)
{
/* Variable declarations */
	uint32_t u32RetVal;
	int16_t i16Parameter[CINSTRUCTIONDECODER_MAX_PARAMETERS];
...
/* Implementation */
	...
	if (i16Parameter[0] < CCONFIG_RELAYMODULE_IOS)
	{
		if ((Timer.GetTiming(i16Parameter[0], CTIMERPIN_TIME_ON, au32Time[0]) == 0) && (Timer.GetTiming(i16Parameter[0], CTIMERPIN_TIME_OFF, au32Time[1]) == 0) && (Timer.GetTiming(i16Parameter[0], CTIMERPIN_TIME_OFFSET, au32Time[2]) == 0)) 
		{
#ifdef CCONFIG_DOCKLIGHT_MODE
			CCONFIG_SERIAL_COM.print("Zeiten von Schaltkanal ");
			CCONFIG_SERIAL_COM.print(i16Parameter[0]);
...
uint32_t CInstructionDecoder::EvaluateInstruction_GSWTF(void)
{
/* Variable declarations */
	uint32_t u32RetVal;
	int16_t i16Parameter[CINSTRUCTIONDECODER_MAX_PARAMETERS];
...
/* Implementation */
	...
	if (i16Parameter[0] < CCONFIG_RELAYMODULE_IOS)
	{
		u32RetVal = Timer.GetTriggerMode((uint8_t)i16Parameter[0], bData);
	
		if (u32RetVal == 0L)
		{
#ifdef CCONFIG_DOCKLIGHT_MODE
			CCONFIG_SERIAL_COM.print("Triggermodus von Schaltkanal ");
			CCONFIG_SERIAL_COM.print(i16Parameter[0]);
#endif /* CCONFIG_DOCKLIGHT_MODE */
...
/* Implementation */
	...
	if (u32Parameter[0] < u32MaxChannel)
	{
		if ((Timer.GetTiming(CALCCHANNEL(u32Parameter[0], bSwitch), CTIMERPIN_TIME_ON, au32Time[0]) == 0) && (Timer.GetTiming(u32Parameter[0], CTIMERPIN_TIME_OFF, au32Time[1]) == 0) && (Timer.GetTiming(u32Parameter[0], CTIMERPIN_TIME_OFFSET, au32Time[2]) == 0))
		{
#ifdef CCONFIG_DOCKLIGHT_MODE
...
uint32_t CInstructionDecoder::EvaluateInstruction_SPSPAF(void)
{
/* Variable declarations */
	uint32_t u32RetVal;
	uint32_t u32Parameter[CINSTRUCTIONDECODER_MAX_PARAMETERS];
...
/* Implementation */
	...
	if (u32Parameter[0] < CCONFIG_POWERSUPPLY_IOS)
	{
		if (Timer.SetAsynchronousMode((uint8_t)u32Parameter[0] + CCONFIG_RELAYMODULE_IOS, u32Parameter[1] != 0L) == 0)
		{
#ifdef CCONFIG_DOCKLIGHT_MODE
			if (u32Parameter[1] == 0L)
			{
				CCONFIG_SERIAL_COM.print("Synchrone Betriebsart fuer Netzteil-Schaltkanal ");
				CCONFIG_SERIAL_COM.print(u32Parameter[0]);
				CCONFIG_SERIAL_COM.println(" gesetzt");
			}
			else
			{
				CCONFIG_SERIAL_COM.print("Asynchrone Betriebsart fuer Netzteil-Schaltkanal ");
				CCONFIG_SERIAL_COM.print(u32Parameter[0]);
				CCONFIG_SERIAL_COM.println(" gesetzt");
			}
#else
...
uint32_t CInstructionDecoder::EvaluateInstruction_SSWAF(void)
{
/* Variable declarations */
	uint32_t u32RetVal;
	uint32_t u32Parameter[CINSTRUCTIONDECODER_MAX_PARAMETERS];
...
/* Implementation */
	...
	if (u32Parameter[0] < CCONFIG_RELAYMODULE_IOS)
	{
		if (Timer.SetAsynchronousMode((uint8_t)u32Parameter[0], u32Parameter[1] != 0L) == 0)
		{
#ifdef CCONFIG_DOCKLIGHT_MODE
			if (u32Parameter[1] == 0L)
			{
				CCONFIG_SERIAL_COM.print("Synchrone Betriebsart fuer Schaltkanal ");
				CCONFIG_SERIAL_COM.print(u32Parameter[0]);
				CCONFIG_SERIAL_COM.println(" gesetzt");
			}
			else
			{
				CCONFIG_SERIAL_COM.print("Asynchrone Betriebsart fuer Schaltkanal ");
				CCONFIG_SERIAL_COM.print(u32Parameter[0]);
				CCONFIG_SERIAL_COM.println(" gesetzt");
			}
#else
...
uint32_t CInstructionDecoder::EvaluateInstruction_SSWC(void)
{
/* Variable declarations */
	uint32_t u32RetVal;
	uint32_t u32Parameter[CINSTRUCTIONDECODER_MAX_PARAMETERS];
...
/* Implementation */
	...
	if (u32Parameter[0] < CCONFIG_RELAYMODULE_IOS)
	{
		if (Timer.SetCounter((uint8_t)u32Parameter[0], u32Parameter[1]) == 0)
		{
#ifdef CCONFIG_DOCKLIGHT_MODE
			CCONFIG_SERIAL_COM.print("Anzahl der Impulse im Impulsbetrieb von Schaltkanal ");
			CCONFIG_SERIAL_COM.print(u32Parameter[0]);
...
uint32_t CInstructionDecoder::EvaluateInstruction_SSWE(void)
{
/* Variable declarations */
	uint32_t u32RetVal;
	int16_t i16Parameter[CINSTRUCTIONDECODER_MAX_PARAMETERS];
...
/* Implementation */
	...
	if (i16Parameter[0] < CCONFIG_RELAYMODULE_IOS)
	{
		if (Timer.SetEnable((uint8_t)i16Parameter[0], (boolean)i16Parameter[1]) == 0)
		{
#ifdef CCONFIG_DOCKLIGHT_MODE
			if (i16Parameter[1] != 0)
			{
...
uint32_t CInstructionDecoder::EvaluateInstruction_SSWIF(void)
{
/* Variable declarations */
	uint32_t u32RetVal;
	uint32_t u32Parameter[CINSTRUCTIONDECODER_MAX_PARAMETERS];
...
/* Implementation */
	...
	if (u32Parameter[0] < CCONFIG_RELAYMODULE_IOS)
	{
		if (Timer.SetImmediateMode((uint8_t)u32Parameter[0], u32Parameter[1] != 0L) == 0)
		{
#ifdef CCONFIG_DOCKLIGHT_MODE
			if (u32Parameter[1] == 0L)
			{
				CCONFIG_SERIAL_COM.print("Ausgabe mit Beginn der On-Phase von Schaltkanal ");
				CCONFIG_SERIAL_COM.print(u32Parameter[0]);
				CCONFIG_SERIAL_COM.println(" gesetzt");
			}
			else
			{
				CCONFIG_SERIAL_COM.print("Unmittelbare Ausgabe von Schaltkanal ");
				CCONFIG_SERIAL_COM.print(u32Parameter[0]);
				CCONFIG_SERIAL_COM.println(" gesetzt");
			}
#else
...
uint32_t CInstructionDecoder::EvaluateInstruction_SSWM(void)
{
/* Variable declarations */
	uint32_t u32RetVal;
	int16_t i16Parameter[CINSTRUCTIONDECODER_MAX_PARAMETERS];
...
/* Implementation */
	...
	if (i16Parameter[0] < CCONFIG_RELAYMODULE_IOS)
	{
		...
		{
			u32RetVal = Timer.SetMode(i16Parameter[0], CTIMERPIN_MODE_OFF);
		
#ifdef CCONFIG_DOCKLIGHT_MODE
			if (u32RetVal == 0L)
			{
				CCONFIG_SERIAL_COM.print("Betriebsmodus von Kanal ");
				CCONFIG_SERIAL_COM.print(i16Parameter[0]);
				CCONFIG_SERIAL_COM.println(" auf 'OFF' gesetzt");
			}
#endif /* CCONFIG_DOCKLIGHT_MODE */
		...
		{
			u32RetVal = Timer.SetMode(i16Parameter[0],CTIMERPIN_MODE_ON);
		...
		{
			u32RetVal = Timer.SetMode(i16Parameter[0],CTIMERPIN_MODE_TOGGLE);
		...
		{
			u32RetVal = Timer.SetMode(i16Parameter[0],CTIMERPIN_MODE_IMPULSE);
...
uint32_t CInstructionDecoder::EvaluateInstruction_SSWNF(void)
{
/* Variable declarations */
	uint32_t u32RetVal;
	uint32_t u32Parameter[CINSTRUCTIONDECODER_MAX_PARAMETERS];
...
/* Implementation */
	...
	if (u32Parameter[0] < CCONFIG_RELAYMODULE_IOS)
	{
		if (Timer.SetInvertedMode((uint8_t)u32Parameter[0], u32Parameter[1] != 0L) == 0)
		{
#ifdef CCONFIG_DOCKLIGHT_MODE
			if (u32Parameter[1] == 0L)
			{
				CCONFIG_SERIAL_COM.print("Nicht invertierte Ausgabe von Schaltkanal ");
				CCONFIG_SERIAL_COM.print(u32Parameter[0]);
				CCONFIG_SERIAL_COM.println(" gesetzt");
			} 
			else
			{
				CCONFIG_SERIAL_COM.print("Invertierte Ausgabe von Schaltkanal ");
				CCONFIG_SERIAL_COM.print(u32Parameter[0]);
				CCONFIG_SERIAL_COM.println(" gesetzt");
			}
#else
...
uint32_t CInstructionDecoder::EvaluateInstruction_SSWT(void)
{
/* Variable declarations */
	uint32_t u32RetVal;
	uint32_t u32Parameter[CINSTRUCTIONDECODER_MAX_PARAMETERS];
...
/* Implementation */
	...
	if (u32Parameter[0] < CCONFIG_RELAYMODULE_IOS)
	{
		u32RetVal = Timer.SetTiming((uint8_t)u32Parameter[0], u32Parameter[1], u32Parameter[2], u32Parameter[3]);
		if (u32RetVal == 0L)
		{
#ifdef CCONFIG_DOCKLIGHT_MODE
			CCONFIG_SERIAL_COM.print("Zeiten von Schaltkanal ");
...
uint32_t CInstructionDecoder::EvaluateInstruction_SSWTF(void)
{
/* Variable declarations */
	uint32_t u32RetVal;
	int16_t i16Parameter[CINSTRUCTIONDECODER_MAX_PARAMETERS];
...
/* Implementation */
	i16Parameter[0] = atoi((char*)m_szParameter[0]);
	i16Parameter[1] = atoi((char*)m_szParameter[1]);
	i16Parameter[2] = atoi((char*)m_szParameter[2]);

	if ((i16Parameter[0] < CCONFIG_RELAYMODULE_IOS) && (i16Parameter[1] < CCONFIG_TRIGGERMODULE_IOS))
	{
		if (Timer.SetTriggerMode((uint8_t)i16Parameter[0], (uint8_t)i16Parameter[1], i16Parameter[2] != 0) == 0)
		{
#ifdef CCONFIG_DOCKLIGHT_MODE
			CCONFIG_SERIAL_COM.print("Trigger-Modus von Schaltkanal ");
			CCONFIG_SERIAL_COM.print(i16Parameter[0]);
			CCONFIG_SERIAL_COM.print(" auf Trigger-Kanal ");
			CCONFIG_SERIAL_COM.print(i16Parameter[1]);
...
/* Implementation */
	...
	while (u8Size_Copy)
	{
		CCONFIG_SERIAL_COM.print(u32Data);
